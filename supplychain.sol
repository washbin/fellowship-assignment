...
contract SupplyChain {
    ...
    */
    modifier forSale(uint256 _sku) {
        require(items[_sku].price != 0, "Unitialized item");
        require(
            items[_sku].state == State.ForSale,
            "Item with given sku is not for sale"
        );
        _;
    }
    modifier sold(uint256 _sku) {
        require(
            ...
            "Item with given sku is not sold"
        );
        _;
    }
    modifier shipped(uint256 _sku) {
        require(
            ...
            "Item with given sku is not shipped"
        );
        _;
    }
    modifier received(uint256 _sku) {
        require(
            ...
            "Item with given sku is not received"
        );
        _;
    }
